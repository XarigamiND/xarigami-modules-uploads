...
// TODO: handle duplicates - cfr. prepare_uploads()
    ...
    if ($ftpLoaded) {
        ...
        if (!$ftpId || !$result) {
            ...
            return xarTplModule('uploads','user','errors',array('errortype' => 'host_connect','var1'=>$msg));
        } else {
    ...
    // Otherwise we have to do it the "hard" way ;-)
        ...
        if (($ftpId = fopen($ftpURI, 'rb')) === FALSE) {
            ...
            if (($tmpId = fopen($tmpName, 'wb')) === FALSE) {
                $msg = xarML('Unable to open temp file to store remote host [#(1):#(2)] file [#(3)]',
                            $uri['host'], $uri['port'], basename($uri['path']));
                return xarTplModule('uploads','user','errors',array('errortype' => 'temp_file','var1'=>$msg));
            } else {
                ...
                do {
                    ...
                    if (0 == strlen($data)) {
                        ...
                        if (fwrite($tmpId, $data, strlen($data)) !== strlen($data)) {
                             return xarTplModule('base','user','errors',array('errortype' => 'not_writeable','var1'=>xarML('temp file') ));
                        }
                    }
                } while (TRUE);
